...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // First, get quick results from limited countries for faster response
    for (const regionConfig of regionsToSearch) {
      const articles = await fetchNewsFromRegion(regionConfig, searchQuery, true);
      allArticles.push(...articles);
    }

    // If we have enough articles, return them quickly
    if (allArticles.length >= 10) {
      console.log(`Quick fetch: Got ${allArticles.length} articles, returning early`);
      ...
      return new Response(
        JSON.stringify({ 
          ...
          message: 'Quick news fetch completed successfully'
        }),
        { 
    ...
    // If we need more articles, continue with full scraping in background
    console.log('Quick fetch insufficient, continuing with full scrape...');
    for (const regionConfig of regionsToSearch) {
      const articles = await fetchNewsFromRegion(regionConfig, searchQuery, false);
      allArticles.push(...articles);
    }
